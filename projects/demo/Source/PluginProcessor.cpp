...
void DemoAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    for(auto j = 0; j < buffer.getNumSamples(); ++j)
    {
        ...
        {
            auto val = *buffer.getReadPointer(i, j);
            float calculated = 0;
            if(val >= 0)
            {
                calculated = fmin(*buffer.getReadPointer(i, j), noteOnVel);
            }
            else
            {
                calculated = fmax(*buffer.getReadPointer(i, j), -noteOnVel);
            }
            
            *buffer.getWritePointer(i, j) = calculated / noteOnVel;
        }
    }
    
    time += buffer.getNumSamples();
}
